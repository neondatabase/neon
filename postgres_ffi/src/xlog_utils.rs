...
mod tests {
    ...
    pub fn test_find_end_of_wal() {
        ...
        let cfg = Conf {
            ...
            datadir: top_path.join("test_output/test_find_end_of_wal"),
        };
        if cfg.datadir.exists() {
            fs::remove_dir_all(&cfg.datadir).unwrap();
        }
        println!("Using initdb from '{}'", cfg.pg_distrib_dir.display());
        println!("Data directory '{}'", cfg.datadir.display());
        cfg.initdb().unwrap();
        let mut srv = cfg.start_server().unwrap();
        let table_name = generate_wal_record_crossing_segment_followed_by_small_one(
            &mut srv.connect_with_timeout().unwrap(),
        )
        .unwrap();
        srv.kill();
        ...
        let waldump_output = cfg.pg_waldump("000000010000000000000002").unwrap().stderr;
