                ...
                if (xlp_info & XLP_FIRST_IS_CONTRECORD) != 0 {
                    ...
                    while xlp_rem_len > 0 {
                        ...
                        if n == pageleft {
                            ...
                            let xlp_magic = LittleEndian::read_u16(&buf[0..2]);
                            let _xlp_info = LittleEndian::read_u16(&buf[2..4]);
                            let xlp_rem_len = LittleEndian::read_u32(
                                &buf[XLP_REM_LEN_OFFS..XLP_REM_LEN_OFFS + 4],
                            );
                            // this is expected in current usage when valid WAL starts after page header
                            println!(
                                "    reading next block with magic={:x} len={:x}",
                                xlp_magic, xlp_rem_len
                            );
                            if xlp_magic != XLOG_PAGE_MAGIC as u16 {
                                trace!(
                                    "invalid WAL file {}.partial magic {} at {:?}",
...
mod tests {
    ...
    pub fn test_find_end_of_wal() {
        ...
        // 1(b). Make some LSN
        let log_path = data_dir.join("initial-queries.log");
        let mut postgres_output = Command::new(top_path.join("tmp_install/bin/postgres"))
            .arg("--single")
            .args(&["-D", data_dir.to_str().unwrap()])
            .args(&["-r", log_path.to_str().unwrap()])
            ...
            .spawn()
            .unwrap();
        let mut postgres_stdin = postgres_output.stdin.take().unwrap();
        //        std::thread::spawn(move || {
        /*            for i in 0..16 {
            postgres_stdin.write_all(format!("select pg_logical_emit_message(false, 'big-1mb-msg-{}', repeat('abcd', 256 * 1024))\n", i).as_bytes()).unwrap();
        }*/
        postgres_stdin.write_all(format!("select pg_logical_emit_message(false, 'big-17mb-msg', concat(repeat('abcd', 17 * 256 * 1024), 'x'))\n").as_bytes()).unwrap();
        postgres_stdin
        ...
        std::thread::sleep(std::time::Duration::from_millis(1000));
        {
            ...
            use nix::unistd::Pid;
            kill(Pid::from_raw(postgres_output.id() as i32), Signal::SIGKILL).unwrap();
        }
        //        std::mem::drop(postgres_stdin);
        //        postgres_output.kill().unwrap();
        let postgres_output = postgres_output.wait_with_output().unwrap();
        if !postgres_output.status.success() {
            std::io::stdout()
                .write_all(&postgres_output.stdout)
                .unwrap();
            std::io::stdout()
                .write_all(&postgres_output.stderr)
                .unwrap();
        }
        //        assert!(postgres_output.status.success());
        //        assert_eq!(log_path.metadata().unwrap().len(), 0);
        ...
        assert_eq!(wal_end, "0/3000000".parse::<Lsn>().unwrap());
        ...
        let waldump_output = Command::new(waldump_path)
            .arg(wal_dir.join("000000010000000000000002"))
            .env_clear()
            .env("LD_LIBRARY_PATH", &lib_path)
            .env("DYLD_LIBRARY_PATH", &lib_path)
            .output()
        ...
        fs::rename(
            ...
            wal_dir.join("000000010000000000000002.partial"),
        )
        .unwrap();
        let (wal_end, tli) = find_end_of_wal(&wal_dir, wal_seg_size, true, Lsn(0)).unwrap();
        let wal_end = Lsn(wal_end);
