...
def test_subscriber_lag(
    ...
    pub_project = neon_create_project(neon_api_key, neon_api_base_url, pg_version)
    pub_project_id = pub_project["project"]["id"]
    neon_wait_for_operation_to_finish(neon_api_key, neon_api_base_url, pub_project_id)
    should_delete_pub = True
    try:
        sub_project = neon_create_project(neon_api_key, neon_api_base_url, pg_version);
        sub_project_id = sub_project["project"]["id"]
        sub_endpoint_id = sub_project["endpoints"][0]["id"]
        neon_wait_for_operation_to_finish(neon_api_key, neon_api_base_url, sub_project_id)
        ...
        try:
            ...
            if should_delete_sub:
                neon_delete_project(neon_api_key, neon_api_base_url, sub_project_id)
    finally:
        if should_delete_pub:
            neon_delete_project(neon_api_key, neon_api_base_url, pub_project_id)
