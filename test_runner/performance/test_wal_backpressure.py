...
def start_heavy_write_workload(env: PgCompare, n_tables: int, scale: int,
                               num_iters: int):
...
def test_heavy_write_workload(pg_compare: PgCompare, n_tables: int, scale: int,
                              num_iters: int):
    env = pg_compare
    ...
    record_read_latency(env, lambda: workload_thread.is_alive(),
                        "SELECT * from t0 where key = 0")
...
def test_pgbench_simple_update_workload(pg_compare: PgCompare, scale: int,
                                        duration: int):
    env = pg_compare
...
def test_pgbench_intensive_init_workload(pg_compare: PgCompare, scale: int):
    ...
    workload_thread = threading.Thread(
        target=start_pgbench_intensive_initialization,
        args=(env, scale, workload_done_event))
    workload_thread.start()

    record_thread = threading.Thread(
        target=record_lsn_write_lag,
        args=(env, lambda: not workload_done_event.is_set()))
    record_thread.start()

    record_read_latency(env, lambda: not workload_done_event.is_set(),
                        "SELECT count(*) from foo")
    workload_thread.join()
    record_thread.join()


def record_lsn_write_lag(env: PgCompare,
                         run_cond: Callable[[], bool],
                         pool_interval: float = 1.0):
    ...
    with env.pg.connect().cursor() as cur:
        ...
        while run_cond():
            ...
            lsn_process_speed = (curr_received_lsn - last_received_lsn) / (1024
                                                                           **2)
            last_received_lsn = curr_received_lsn

            curr_pg_flush_lsn = lsn_from_hex(res[2])
            lsn_produce_speed = (curr_pg_flush_lsn - last_pg_flush_lsn) / (1024
                                                                           **2)
...
def record_read_latency(env: PgCompare,
    ...
    env.zenbenchmark.record("read_latency_max", max(read_latencies), "s",
                            MetricReport.LOWER_IS_BETTER)
    env.zenbenchmark.record("read_latency_avg",
                            statistics.mean(read_latencies), "s",
                            MetricReport.LOWER_IS_BETTER)
    env.zenbenchmark.record("read_latency_stdev",
                            statistics.stdev(read_latencies), "s",
                            MetricReport.LOWER_IS_BETTER)
