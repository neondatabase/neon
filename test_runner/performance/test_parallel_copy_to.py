...
async def copy_test_data_to_table(pg: PgProtocol, worker_id: int,
                                  table_name: str):
    buf = BytesIO()
    for i in range(1000):
        buf.write(
...
async def parallel_load_different_tables(pg: PgProtocol, n_parallel: int):
    ...
    for worker_id in range(n_parallel):
        worker = copy_test_data_to_table(pg, worker_id,
                                         f"copytest_{worker_id}")
...
def test_parallel_copy_different_tables(neon_with_baseline: PgCompare,
                                        n_parallel=5):
