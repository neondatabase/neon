...
# This test used to be part of the regression test suite.
# It became flaky in CI starting 2025-03-10 and was disabled on 2025-04-02.
# The flaky results have in common that pagebench reported severe latency outliers
# (p99.9 and p99.99 at > 100ms whereas expected is 10ms as per the `rate_limit_rps` variable).
# The suspicion is that this was due to noisy neighbor tests (CPU starvation).
# So, we moved the test into the performance test suite, which runs tests sequentially.
# Normally we would insist on fixing the timigng dependence of the test instead, but,
# the throttle is not architected to a
