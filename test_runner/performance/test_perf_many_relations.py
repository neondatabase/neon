...
def test_perf_many_relations(neon_with_baseline: PgCompare, num_relations: int):
    ...
    while num_relations_remaining > 0:
        ...
        log.info(f"Creating {num_relations_to_create} child tables in partitioned parent table '{parent_table_name}'")
        ...
        log.info(sql)
        with env.zenbenchmark.record_duration(f"CREATE_TABLE/{current_table}/{num_relations_to_create}"):
            env.pg_bin.run_capture(["psql", env.pg.connstr(), "-c", sql])
