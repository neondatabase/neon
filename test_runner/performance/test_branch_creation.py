...
def _record_branch_creation_durations(neon_compare: NeonCompare,
                                      durs: List[float]):
    neon_compare.zenbenchmark.record("branch_creation_duration_max", max(durs),
                                     "s", MetricReport.LOWER_IS_BETTER)
    neon_compare.zenbenchmark.record("branch_creation_duration_avg",
                                     statistics.mean(durs), "s",
                                     MetricReport.LOWER_IS_BETTER)
    neon_compare.zenbenchmark.record("branch_creation_duration_stdev",
                                     statistics.stdev(durs), "s",
                                     MetricReport.LOWER_IS_BETTER)
...
def test_branch_creation_heavy_write(neon_compare: NeonCompare,
                                     n_branches: int):
    ...
    threads.append(
        threading.Thread(target=run_pgbench, args=("b0", ), daemon=True))
    threads[-1].start()
    ...
    for i in range(n_branches):
        ...
        env.neon_cli.create_branch("b{}".format(i + 1),
                                   "b{}".format(p),
                                   tenant_id=tenant)
        ...
        threads.append(
            threading.Thread(target=run_pgbench,
                             args=(f"b{i+1}", ),
                             daemon=True))
        threads[-1].start()
