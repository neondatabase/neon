...
def test_broken_timeline(neon_env_builder: NeonEnvBuilder):
    ...
    metadata_path = "{}/tenants/{}/timelines/{}/metadata".format(
        env.repo_dir, tenant1, timeline1)
    ...
    timeline_path = "{}/tenants/{}/timelines/{}/".format(
        env.repo_dir, tenant2, timeline2)
    ...
    timeline_path = "{}/tenants/{}/timelines/{}/".format(
        env.repo_dir, tenant3, timeline3)
    for filename in os.listdir(timeline_path):
        if filename.startswith("00000"):
    ...
    for n in range(1, 4):
        ...
        with pytest.raises(Exception,
                           match="Cannot load local timeline") as err:
            pg.start()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(env.neon_cli.create_timeline,
                            f"test-create-multiple-timelines-{i}", tenant_id)
            for i in range(4)
        ]
...
def test_fix_broken_timelines_on_startup(neon_simple_env: NeonEnv):
    ...
    env.pageserver.safe_psql(
        "failpoints before-checkpoint-new-timeline=return")
    with pytest.raises(Exception, match="before-checkpoint-new-timeline"):
        _ = env.neon_cli.create_timeline("test_fix_broken_timelines",
                                         tenant_id)
    ...
    with pytest.raises(Exception,
                       match=f"Failed to get repo for tenant {tenant_id.hex}"):
