...
def test_ondemand_download_large_rel(
    ...
    with pg.cursor() as cur:
        ...
        cur.execute(f'''CREATE TABLE tbl AS SELECT g as id, 'long string to consume some space' || g
    ...
    before_downloads = int(
        client.get_metric_value("pageserver_layers_downloaded_total",
                                UUID(tenant_id),
                                UUID(timeline_id)))
...
def test_ondemand_download_timetravel(
    ...
    for (checkpoint_number, lsn) in lsns:
        ...
        with pg_old.cursor() as cur:
            ...
            assert query_scalar(
                cur,
                f"select count(*) from testtab where checkpoint_number<>{checkpoint_number}") == 0
        after_downloads = int(
            client.get_metric_value("pageserver_layers_downloaded_total",
                                    UUID(tenant_id),
                                    UUID(timeline_id)))
