...
def test_metrics_normal_work(neon_env_builder: NeonEnvBuilder):
    ...
    timeline_1 = env.neon_cli.create_timeline("test_metrics_normal_work",
                                              tenant_id=tenant_1)
    timeline_2 = env.neon_cli.create_timeline("test_metrics_normal_work",
                                              tenant_id=tenant_2)

    pg_tenant1 = env.postgres.create_start("test_metrics_normal_work",
                                           tenant_id=tenant_1)
    pg_tenant2 = env.postgres.create_start("test_metrics_normal_work",
                                           tenant_id=tenant_2)
    ...
    for sk in env.safekeepers:
        collected_metrics[f"safekeeper{sk.id}"] = sk.http_client(
        ).get_metrics_str()
    ...
    all_metrics = [
        parse_metrics(m, name) for name, m in collected_metrics.items()
    ]
    ...
    ttids = [
        {
            "tenant_id": tenant_1.hex,
            "timeline_id": timeline_1.hex
        },
        {
            "tenant_id": tenant_2.hex,
            "timeline_id": timeline_2.hex
        },
    ]
    ...
    for tt in ttids:
        ...
        ps_lsn = int(
            ps_metrics.query_one("pageserver_last_record_lsn",
                                 filter=tt).value)
        sk_lsns = [
            int(sk.query_one("safekeeper_commit_lsn", filter=tt).value)
    ...
    for metrics in all_metrics:
        ...
        log.info(
            f"process_threads: {int(metrics.query_one('process_threads').value)}"
        )
        log.info(
        ...
        log.info(
            f"process_open_fds: {int(metrics.query_one('process_open_fds').value)}"
        )
        log.info(
            f"process_max_fds: {int(metrics.query_one('process_max_fds').value)}"
        )
        log.info(
