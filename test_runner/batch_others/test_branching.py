...
def test_branching_with_pgbench(neon_simple_env: NeonEnv, pg_bin: PgBin,
                                n_branches: int, scale: int, ty: str):
    env = neon_simple_env
    ...
    threads.append(
        threading.Thread(target=run_pgbench, args=(pgs[0], ), daemon=True))
    threads[-1].start()
    ...
    for i in range(n_branches):
        ...
        pgs.append(
            env.postgres.create_start("b{}".format(i + 1), tenant_id=tenant))

        threads.append(
            threading.Thread(target=run_pgbench, args=(pgs[-1], ),
                             daemon=True))
        threads[-1].start()
