...
def test_read_validation(neon_simple_env: NeonEnv):
    ...
    with closing(pg.connect()) as con:
        with con.cursor() as c:
            ...
            c.execute(
                "create table foo (c int) with (autovacuum_enabled = false)")
            c.execute("insert into foo values (1)")

            c.execute(
                "select lsn, lower, upper from page_header(get_raw_page('foo', 'main', 0));"
            )
            first = c.fetchone()
            assert first is not None

            relfilenode = query_scalar(
                c, "select relfilenode from pg_class where relname = 'foo'")
            ...
            log.info(
                "Clear buffer cache to ensure no stale pages are brought into the cache"
            )
            ...
            log.info(
                "Cache is clear, reading latest page version without cache")
            ...
            log.info(
                "Validation page inspect won't allow reading pages of dropped relations"
            )
            try:
                c.execute(
