...
def test_timeline_delete(neon_simple_env: NeonEnv):
    ...
    with pytest.raises(NeonPageserverApiException, match="timeline not found"):
        ps_http.timeline_delete(tenant_id=env.initial_tenant,
    ...
    with pytest.raises(
            ...
            match=
            f"Tenant {invalid_tenant_id.hex} not found in local tenant state",
    ):
        ps_http.timeline_delete(tenant_id=invalid_tenant_id,
                                timeline_id=invalid_timeline_id)
    ...
    parent_timeline_id = env.neon_cli.create_branch(
        "test_ancestor_branch_delete_parent", "empty")
    ...
    with pytest.raises(
        ...
        timeline_path = (env.repo_dir / "tenants" / env.initial_tenant.hex /
                         "timelines" / parent_timeline_id.hex)
    ...
    timeline_path = (env.repo_dir / "tenants" / env.initial_tenant.hex /
                     "timelines" / leaf_timeline_id.hex)
    assert timeline_path.exists()
    ...
    wait_until(
        ...
        interval=0.2,
        func=lambda: ps_http.timeline_delete(env.initial_tenant,
                                             leaf_timeline_id),
    )
