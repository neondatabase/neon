...
def test_timeline_size(neon_simple_env: NeonEnv):
    ...
    timeline_details = assert_timeline_local(client, env.initial_tenant,
                                             new_timeline_id)
    assert (timeline_details["local"]["current_logical_size"] ==
    ...
    with closing(pgmain.connect()) as conn:
        with conn.cursor() as cur:
            ...
            res = assert_timeline_local(client, env.initial_tenant,
                                        new_timeline_id)
            local_details = res["local"]
            assert (local_details["current_logical_size"] ==
...
def test_timeline_size_quota(neon_env_builder: NeonEnvBuilder):
    ...
    with closing(pgmain.connect()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "CREATE EXTENSION neon")  # TODO move it to neon_fixtures?
...
def test_timeline_physical_size_post_checkpoint(neon_simple_env: NeonEnv):
    env = neon_simple_env
    new_timeline_id = env.neon_cli.create_branch(
        "test_timeline_physical_size_post_checkpoint")
    ...
    env.pageserver.safe_psql(
        f"checkpoint {env.initial_tenant.hex} {new_timeline_id.hex}")
...
def test_timeline_physical_size_post_compaction(
    ...
    env.pageserver.safe_psql(
        f"checkpoint {env.initial_tenant.hex} {new_timeline_id.hex}")
    env.pageserver.safe_psql(
        f"compact {env.initial_tenant.hex} {new_timeline_id.hex}")
...
def test_timeline_physical_size_post_gc(neon_env_builder: NeonEnvBuilder):
    ...
    env.pageserver.safe_psql(
        f"do_gc {env.initial_tenant.hex} {new_timeline_id.hex} 0")
...
def test_timeline_size_metrics(neon_simple_env: NeonEnv):
    ...
    assert math.isclose(tl_logical_size_metric,
                        32 * 1024 * 1024,
                        abs_tol=2 * 1024 * 1024)
    ...
    dbsize_sum = pg.safe_psql(
        "select sum(pg_database_size(oid)) from pg_database")[0][0]
    assert math.isclose(dbsize_sum,
                        tl_logical_size_metric,
                        abs_tol=2 * 1024 * 1024)
...
def test_tenant_physical_size(neon_simple_env: NeonEnv):
    ...
    def get_timeline_physical_size(timeline: UUID):
        res = client.timeline_detail(
            tenant, timeline, include_non_incremental_physical_size=True)
    ...
    for i in range(10):
        ...
        timeline = env.neon_cli.create_branch(f"test_tenant_physical_size_{i}",
                                              tenant_id=tenant)
        pg = env.postgres.create_start(f"test_tenant_physical_size_{i}",
                                       tenant_id=tenant)
...
def assert_physical_size(env: NeonEnv, tenant_id: UUID, timeline_id: UUID):
    ...
    assert res["local"]["current_physical_size"] == get_timeline_dir_size(
        timeline_path)
