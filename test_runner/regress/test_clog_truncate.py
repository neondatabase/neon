...
def test_clog_truncate(neon_simple_env: NeonEnv):
    ...
    log.info("%s", "pg_xact_0000_path = {pg_xact_0000_path}")

    while os.path.isfile(pg_xact_0000_path):
        log.info("%s", "file exists. wait for truncation: {pg_xact_0000_path=}")
        time.sleep(5)
    ...
    log.info("%s", "create branch at lsn_after_truncation {lsn_after_truncation}")
    ...
    log.info("%s", "pg_xact_0000_path_new = {pg_xact_0000_path_new}")
