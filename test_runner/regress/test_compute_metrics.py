...
class SqlExporterRunner:
    def __init__(self, test_output_dir: Path) -> None:
        self.__test_output_dir = test_output_dir
    ...
    def __exit__(
        ...
        try:
            ...
            with open(self.__test_output_dir / "sql_exporter.stderr", "w", encoding="utf-8") as f:
                f.write(self.logs())
...
if SQL_EXPORTER is None:
    ...
    class SqlExporterContainerRunner(SqlExporterRunner):
        def __init__(
            ...
            super().__init__(test_output_dir)
        ...
        @override
        def logs(self) -> str:
            return self.__container.get_logs()[1].decode()  # type: ignore
    ...
    class SqlExporterNativeRunner(SqlExporterRunner):
        def __init__(
            ...
            self.__sql_exporter_port = sql_exporter_port
            self.__proc: subprocess.Popen[str]

        @override
        def logs(self) -> str:
            assert self.__proc.stderr is not None
            stderr = self.__proc.stderr.read()
        ...
        def start(self) -> None:
            ...
            self.__proc = subprocess.Popen(
                [
                    ...
                    f"-web.listen-address=:{self.__sql_exporter_port}",
                ],
            ...
            while True:
                ...
                if f'level=info msg="Listening on" address=[::]:{self.__sql_exporter_port}' in line:
                    break
