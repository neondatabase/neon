...
SQL_EXPORTER_IN_PATH = shutil.which("sql_exporter")

if not SQL_EXPORTER_IN_PATH:
    ...
    class SqlExporterContainer(DockerContainer):  # type: ignore
        ...
        @override
        def stop(self, force: bool = True, delete_volume: bool = True):
            with (
                open(self.__logs_dir / "sql_exporter.stdout", "w", encoding="utf-8") as stdout,
                open(self.__logs_dir / "sql_exporter.stderr", "w", encoding="utf-8") as stderr,
            ):
                logs = self.get_logs()
                stdout.write(logs[0].decode())
                stderr.write(logs[1].decode())
    ...
    @contextmanager
    def sql_exporter(
        logs_dir: Path, config_file: Path, port: int
    ) -> Iterator[subprocess.Popen[bytes]]:
        with (
            open(logs_dir / "sql_exporter.stdout", "w", encoding="utf-8") as stdout,
            open(logs_dir / "sql_exporter.stderr", "w", encoding="utf-8") as stderr,
        ):
...
def test_sql_exporter_metrics_e2e(
    ...
    with open(config_file, "w", encoding="utf-8") as o:
        config = evaluate_config(
            ...
            collector_file=collector_file if SQL_EXPORTER_IN_PATH is None else collector_file.name,
            connstr=connstr,
        )
        o.write(config)
    ...
    if not SQL_EXPORTER_IN_PATH:
        ...
        sql_exporter(test_output_dir, config_file, sql_exporter_port)
