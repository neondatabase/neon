...
def test_forward_compatibility(
    ...
    """
    assert (
        compatibility_neon_binpath is not None
    ), "Please set the environment variable COMPATIBILITY_NEON_BIN"
    assert (
        compatibility_pg_distrib_dir is not None
    ), "Please set the environment variable COMPAT_POSTGRES_DISTRIB_DIR"
    ...
    try:
        ...
        neon_env_builder.neon_binpath = compatibility_neon_binpath
        neon_env_builder.pg_distrib_dir = compatibility_pg_distrib_dir
...
def test_versions_mismatch(
    ...
    assert compatibility_snapshot_dir_env is not None, (
        ...
        "path generateted by test_create_snapshot (ideally generated by the previous version of Neon)"
    )
    compatibility_snapshot_dir = Path(compatibility_snapshot_dir_env).resolve()
