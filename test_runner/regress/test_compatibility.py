...
def test_versions_mismatch(neon_env_builder: NeonEnvBuilder, test_output_dir: Path, pg_version: PgVersion, combination):
    compatibility_snapshot_dir_env = os.environ.get("COMPATIBILITY_SNAPSHOT_DIR")
    assert (
            compatibility_snapshot_dir_env is not None
    ), f"COMPATIBILITY_SNAPSHOT_DIR is not set. It should be set to `compatibility_snapshot_pg{pg_version.v_prefixed}` path generateted by test_create_snapshot (ideally generated by the previous version of Neon)"
    ...
    compatibility_neon_bin = Path(compatibility_neon_bin_env).resolve()
    compatibility_neon_dir = Path(os.path.abspath(compatibility_neon_bin / '..' / '..')).resolve()
    binaries = {
        'storage_controller': ['target/release/storage_controller'],
        'storage_broker': ['target/release/storage_broker'],
        'compute': ['target/release/compute_ctl', 'pg_install'],
        'safekeeper': ['target/release/safekeeper'],
        'pageserver': ['target/release/pageserver', 'target/release/pagectl'],
    }
    target_dir = Path(os.path.join(test_output_dir, 'neon', 'target', 'release'))
    os.makedirs(target_dir)
    for component, paths in binaries.items():
        directory = Path('.') if bool(combination & bitmap[component]) else compatibility_neon_dir
        log.info('Directory: %s', directory)
        for path in paths:
            filename = os.path.basename(path)
            log.info("%s %s", path, filename)
            if os.path.isdir(path):
                os.symlink(os.path.abspath(path), target_dir / '..' / '..' / filename, True)
            else:
                log.info('Target: %s', os.path.join(target_dir, filename))
                os.link(directory / path, target_dir / filename)
    neon_env_builder.neon_binpath = target_dir
    neon_env_builder.pg_distrib_dir = compatibility_neon_dir / 'pg_install'
    neon_env_builder.num_safekeepers = 3
    #env = neon_env_builder.init_start()
    env = neon_env_builder.from_repo_dir(
        compatibility_snapshot_dir / "repo",
    ...
    env.start()
    check_neon_works(env, test_output_dir, compatibility_snapshot_dir / 'dump.sql', test_output_dir / 'repo')
