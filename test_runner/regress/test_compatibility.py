...
def test_create_snapshot(neon_env_builder: NeonEnvBuilder, pg_bin: PgBin, test_output_dir: Path):
    ...
    neon_env_builder.pg_version = "15"
    ...
    shutil.copytree(test_output_dir, test_output_dir / "compatibility_snapshot_pg15")
    # Directory `test_output_dir / "compatibility_snapshot_pg15"` is uploaded to S3 in a workflow, keep the name in sync with it
...
def test_backward_compatibility(
    ...
    ), "COMPATIBILITY_SNAPSHOT_DIR is not set. It should be set to `compatibility_snapshot_pg15` path generateted by test_create_snapshot (ideally generated by the previous version of Neon)"
...
def test_forward_compatibility(
    ...
    compatibility_snapshot_dir = (
        test_output_dir.parent / "test_create_snapshot" / "compatibility_snapshot_pg15"
    )
    # Copy the snapshot to current directory, and prepare for the test
    prepare_snapshot(
        from_dir=compatibility_snapshot_dir,
