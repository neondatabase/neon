...
@pytest.mark.parametrize("pageserver_aux_file_v2", [AuxFileStore.V1, AuxFileStore.V2])
def test_aux_file_v2_flag(neon_simple_env: NeonEnv, pageserver_aux_file_v2: AuxFileStore):
    env = neon_simple_env
    with env.pageserver.http_client() as client:
        tenant_config = client.tenant_config(env.initial_tenant).effective_config
        if tenant_config["switch_to_aux_file_v2"]:
            assert pageserver_aux_file_v2 == AuxFileStore.V2
        else:
            assert pageserver_aux_file_v2 == AuxFileStore.V1
