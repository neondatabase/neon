...
@pytest.mark.skipif(not USE_LFC, reason="LFC is disabled, skipping")
def test_replica_promote_old_primary_starts(neon_simple_env: NeonEnv):
    """
    Test old primary errors out on starting when secondary is promoted
    """
    env: NeonEnv = neon_simple_env
    primary: Endpoint = env.endpoints.create_start(branch_name="main", endpoint_id="primary")
    secondary: Endpoint = env.endpoints.new_replica_start(origin=primary, endpoint_id="secondary")
    ...
    # walproposer should panic while comparing propTermStartLsn and basebackup LSN
