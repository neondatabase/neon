...
def test_lfc_working_set_approximation(neon_simple_env: NeonEnv):
    ...
    log.info("%s", "preparing some data in {endpoint.connstr()}")
...
"""
    ...
    log.info("%s", "pgbench_accounts has {pages} pages, resetting working set to zero")
    ...
    log.info("%s", "working set size after sequential scan on pgbench_accounts {blocks}")
    ...
    blocks = query_scalar(cur, "select approximate_working_set_size(true)")
    log.info("%s", "working set size after some index access of a few select pages only {blocks}")
    assert blocks < 10
