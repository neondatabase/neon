...
def decompress_zstd(
    ...
    log.info("%s", "decompressing zstd to: {output_dir}")
...
def test_wal_restore_initdb(
    ...
    with VanillaPostgres(
        ...
        restored.start()
        Lsn(restored.safe_psql("SELECT pg_current_wal_flush_lsn()", user="cloud_admin")[0][0])
        log.info("%s", "original lsn: {original_lsn}, restored lsn: {restored_lsn}")
        assert restored.safe_psql("select count(*) from t", user="cloud_admin") == [(300000,)]
