...
def test_metrics_normal_work(neon_env_builder: NeonEnvBuilder):
    ...
    for tt in ttids:
        log.info("%s", "Checking metrics for {tt}")
        ...
        log.info("%s", "ps_lsn: {ps_lsn}")
        log.info("%s", "sk_lsns: {sk_lsns}")
    ...
    for metrics in all_metrics:
        log.info("%s", "Checking common metrics for {metrics.name}")

        log.info(
            f"process_cpu_seconds_total: {metrics.query_one('process_cpu_seconds_total').value}"
        )
        log.info("%s", "process_threads: {int(metrics.query_one('process_threads').value)}")
        log.info(
        ...
        log.info("%s", "process_open_fds: {int(metrics.query_one('process_open_fds').value)}")
        log.info("%s", "process_max_fds: {int(metrics.query_one('process_max_fds').value)}")
        log.info(
    ...
    for io_direction in ["read", "write"]:
        ...
        log.info("%s", "Pageserver {io_direction} bytes from another AZ: {total_bytes}")
    ...
    def create_bg(delay_ms):
        ...
        try:
            ...
            if e.status_code == 409:
                log.info("%s", "delay_ms={delay_ms} 409")
                pass
...
def test_pageserver_metrics_many_relations(neon_env_builder: NeonEnvBuilder):
    ...
    log.info("%s", "pageserver_directory_entries_count metric value: {directory_entries_count}")
    ...
    assert counts
    log.info("%s", "directory counts: {counts}")
