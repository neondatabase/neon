...
def test_vm_bit_clear(neon_simple_env: NeonEnv):
    ...
    cur.execute("CREATE TABLE vmtest_update2 (id integer PRIMARY KEY, filler text)")
    cur.execute("INSERT INTO vmtest_update2 SELECT g, '' FROM generate_series(1, 1000) g")
    cur.execute("VACUUM FREEZE vmtest_update2")
    ...
    cur.execute("INSERT INTO vmtest_update2 VALUES (9999, 'x')");
    # Clears the VM bit on the old page
    cur.execute("UPDATE vmtest_update2 SET id = 5000, filler=repeat('x', 200) WHERE id = 1")
    ...
    assert cur.fetchall() == []
    cur.execute("SELECT id FROM vmtest_update WHERE id = 1")
    assert cur.fetchall() == []
    cur.execute("SELECT id FROM vmtest_update2 WHERE id = 1")
    assert cur.fetchall() == []
    ...
    cur_new.execute("SELECT id FROM vmtest_update WHERE id = 1")
    assert cur_new.fetchall() == []
    cur_new.execute("SELECT id FROM vmtest_update2 WHERE id = 1")
    assert cur_new.fetchall() == []
