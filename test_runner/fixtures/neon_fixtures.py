...
def pytest_configure(config: "Config"):
    ...
    assert (
        numprocesses is None or BASE_PORT + numprocesses * WORKER_PORT_NUM > 32768
    ...
    psql_bin_path = os.path.join(pg_distrib_dir, f"v{default_pg_version}", "bin/psql")
    postgres_bin_path = os.path.join(pg_distrib_dir, f"v{default_pg_version}", "bin/postgres")

    if os.getenv("REMOTE_ENV"):
        # When testing against a remote server, we only need the client binary.
        assert os.path.exists(psql_bin_path), f'psql not found at "{psql_bin_path}"'
    else:
        assert os.path.exists(postgres_bin_path), f'postgres not found at "{postgres_bin_path}"'
    ...
    assert os.path.exists(
        os.path.join(neon_binpath, "pageserver")
    ), f'neon binaries not found at "{neon_binpath}"'


def profiling_supported() -> bool:
...
@pytest.fixture(scope="session")
def default_broker(request: "FixtureRequest", port_distributor: PortDistributor) -> Iterator[Etcd]:
...
class PgProtocol:
    ...
    def conn_options(self, **kwargs: Any) -> Dict[str, Any]:
        result: Dict[str, Any] = self.default_options.copy()
        if "dsn" in kwargs:
...
class AuthKeys:
    ...
    def generate_management_token(self) -> str:
        token = jwt.encode({"scope": "pageserverapi"}, cast(str, self.priv), algorithm="RS256")
...
@pytest.fixture(scope="function")
def link_proxy(port_distributor: PortDistributor) -> Iterator[NeonProxy]:
...
def get_test_output_dir(request: "FixtureRequest") -> Path:
...
def test_output_dir(request: "FixtureRequest") -> Iterator[Path]:
