...
- https://github.com/neondatabase/neon/issues/1751
  GC and compaction file activities are not well known outside of their code, causing race bugs
...
- more issues were brought in discussions, but apparently they were too specific to the code to mention them in the issues.
  ...
  In fact, we have zero Rust tests that cover the case of syncrhonizing more than one module's data.
...
The previous data diagram shows most of the timeline data:
...
- a `metadata` file, on the FS
  Updated after every checkpoint with the never `disk_consistent_lsn` and `latest_gc_cutoff_lsn` values. Used to quickly restore timeline relations and basic metadata on pageserver restart.
...
- an `index_part.json` file, on the remote storage
  ...
  Used to avoid potentially slow and expencive `S3 list` command, updated by the remotes storage sync thread after every operation with the remote layer files.
...
Yet, there's a knowledge on a tenant as a group of timelines, belonging to a single user which is used in GC and compaction tasks, run on every tenant.
...
Currently we have multiple places where timeline data is stored:

- `tenant_mgr` [holds](https://github.com/neondatabase/neon/blob/main/pageserver/src/tenant_mgr.rs#L43) a static `static ref TENANTS: RwLock<HashMap<ZTenantId, Tenant>>` with the `Tenant` having the `local_timelines: HashMap<ZTimelineId, Arc<DatadirTimelineImpl>>` inside

- same `Tenant` above has actually two references to timelines: another via its `repo: Arc<RepositoryImpl>` with `pub type RepositoryImpl = LayeredRepository;` that [holds](https://github.com/neondatabase/neon/blob/main/pageserver/src/layered_repository.rs#L178) `Mutex<HashMap<ZTimelineId, LayeredTimelineEntry>>`

- `RemoteTimelineIndex` [contains](https://github.com/neondatabase/neon/blob/main/pageserver/src/storage_sync/index.rs#L80) the metadata about timelines on the remote storage (S3) for sync reasons and possible HTTP API queries

- `walreceiver` [stores](https://github.com/neondatabase/neon/blob/main/pageserver/src/walreceiver.rs#L60) the metadata for possible HTTP API queries and its [internal state](https://github.com/neondatabase/neon/blob/main/pageserver/src/walreceiver/connection_manager.rs#L245) with a reference to the timeline, its current connections and etcd subscription (if any)

- `PageCache` contains timeline-related data, and is created globally for the whole pageserver

- implicitly, we also have files on local FS, that contain timeline state. We operate on those files and for some operations (GC, compaction) we now acquire a [file lock](https://github.com/neondatabase/neon/blob/main/pageserver/src/layered_repository.rs#L169) in the corresponding timeline repository, `file_lock: RwLock<()>`.
...
Some modules are in transition, started as async "blocking" tasks and being fully synchronous in their entire code below the start. Current idea is to transfer them to the sync further, but it's not yet done.
...
1. [HTTP requests](https://github.com/neondatabase/neon/blob/main/pageserver/src/http/routes.rs)
...
It does not mean that the operations themselves are short, e.g. `tenant_attach_handler` downloads multiple files from the remote storage which might take time, yet the data access is not used as `Mutex<()>` locks to orchestrate the data access in specific order.
Multiple locks may be needed
...
"Layer data access" describes both complex cases in more details later.

2. [libpq requests](https://github.com/neondatabase/neon/blob/main/pageserver/src/page_service.rs)

Is the main interface of pageserver, intended to handle libpq (and similar) requests.
Operates on `LayeredTimeline` and, lower, `LayerMap` layers; all timelines accessed during the operation are loaded into memory immediately (if not loaded already), operations bail if timeline load errors.
...
"Metadata" retrieval methods, that still require internal knowledge.
...
Apart from loading into memory, every timeline layer has to be accessed using specific primitives, especially if a write operations happens: otherwise, GC or compaction might spoil the data. User API is implicitly affected by this synchronization during branching, when a GC has to be orchestrated properly before the new timeline could be branched off the existing one.
See "Layer data access" section for the united synchronization diagram on the topic.

3. internal access

Entities within pageserver to update files on local FS and remote storage, metadata in memory; has to internal data for those operations.
...
Fills memory with data, eventually triggering a checkpoint task that creates a new layer file in the local FS and chedules a remote storage sync upload task.
...
Periodic, short-lived tasks to generate a new layer file in the FS. Requires low level synchronization in the end, when the layer is being registered after creating and has additional mode to mode to ensure only one concurrent compaction happens at a time.
...
`RwLock<LayerMap>` is almost an exception to the previous rule, since it does taken in a multiple places to ensure all layers are inserted correctly.
...
Note though, that due to current "working with loaded timeline only", overwhelming amount of the locks taken on the struct are `.write()` locks, not the `.read()` ones.
To simplify the diagrams, these accesses are now considered "fast" data access, not the synchornization attempts.
...
- `layer_flush_lock: Mutex<()>` and layer flushing seems to be slightly bloated with various ways to create a layer on disk and write it in memory
  ...
  Yet the latter is not really done consistently, since remote storage sync manages to download and register the new layers without thouching the locks
...
- it's not described on the current diagram, but libpq checkpoint call actually calls compaction twice, once due to layer flushing, once after that flush due to explicit call.
...
- `partitioning: Mutex<(KeyPartitioning, Lsn)>` lock is a data sync lock that's not used to synchronize the tasks, yet is very similar to what `write_lock` and `layer_flush_lock` do. The notable difference is that the locks are in different (albeit, nested) structs.
...
### How to stucture timeline data access better

- adjust tenant state handling

Current [`TenantState`](https://github.com/neondatabase/neon/blob/main/pageserver/src/tenant_mgr.rs#L108) [changes](https://github.com/neondatabase/neon/blob/main/pageserver/src/tenant_mgr.rs#L317) mainly indicates whether GC and compaction tasks are running or not; another state, `Broken` shows only in case any timeline does not load during startup.

We could start both tasks at the time the tenant is started and adjust them to throttle/sleep on timeline absence and wake up when tie first one added and never fail on timeline load.
The latter becomes more important on download on demand, since we won't have the entire timeline in reach to verify its correcness. Moreover, if any network connection happens, the timeline could fail temporarily and entire tenant should be marked as broken due to that.

Since no software checks the tenant state via HTTP API currently, it makes sense to remove the whole state entirely and don't write the code to synchronize its changes.

- remove the "unloaded" status for the timeline

Current approach to timeline management [assumes](https://github.com/neondatabase/neon/blob/main/pageserver/src/layered_repository.rs#L486-L493)
...
There are a few timeline methods that return timeline metadata without loading its layers, but such metadata also comes from the `metadata` fs file, not the layers.

With the layer on-demand download, it's not feasible anymore to wait for the entire layer map to be loaded into the memory, since it might not even be available on the local FS when requested.

To accomodate to that and move away from the redundant status, a timeline should always be "loaded" with its metadata read from the disk and its layer map prepared to be downloaded when requested, per layer.
Currently existing HTTP API operations such as tenant attach are expected to stay the same API-wise, but finishing sooner since the full layer map load is done no more.
...
In particular, layer files need to be better prepared for furure download on demand functionality, where every layer could be dynamically loaded and unloaded from memory and local FS.
...
One of the solutions to the issue is already being developed for the remote storage sync: [SyncQueue](https://github.com/neondatabase/neon/blob/main/pageserver/src/storage_sync.rs#L463)
The queue is able to batch CRUD layer operations (both for local and fs contexts) and reorder them to increase the sync speed.
...
more conveninent for the data sync system inside.
...
`page_cache` is proposed to placed into the same `data_storage` since it contains tenant timelines' data: this way, all metadata and data is in the same struct, simplifying things with Rust's borrow checker and allowing is to do care about data sync withing a single module.

`task_manager` is related to data storage and manages all tenant and timeline tasks, manages shared resources (runtimes, threadpools, etcd connection, etc.) and synchronizes tasks.
...
`LayeredTimeline` is still split into two parts, more high-level with whatever promitives needed to sync its state, and the actual state storage with `LayerMap` and other low level entities.
...
It raises the question where this lock has to be placed, it's the only place that requires pausing a GC task during external, HTTP request handling.
The right place for the lock seems to be the `task_manager` that could manage GC in more fine-grained way to accomodate the incoming branching request.
