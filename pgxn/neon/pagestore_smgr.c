...
nm_unpack_response(StringInfo s)
{
	...
	resp_hdr.tag = tag;
	resp_hdr.u.reqid = pq_getmsgint64(s);
	...
	switch (tag)
	{
		...
		case T_NeonNblocksResponse:
			{
				NeonNblocksResponse *msg_resp = memalloc(sizeof(NeonNblocksResponse));
		...
		case T_NeonGetPageResponse:
			{
				...
				msg_resp = memalloc(PS_GETPAGERESPONSE_SIZE);
				NInfoGetSpcOid(msg_resp->req.rinfo) = pq_getmsgint(s, 4);
				NInfoGetDbOid(msg_resp->req.rinfo) = pq_getmsgint(s, 4);
				NInfoGetRelNumber(msg_resp->req.rinfo) = pq_getmsgint(s, 4);
				msg_resp->req.forknum = pq_getmsgbyte(s);
		...
		case T_NeonDbSizeResponse:
			{
				NeonDbSizeResponse *msg_resp = memalloc(sizeof(NeonDbSizeResponse));
		...
		case T_NeonErrorResponse:
			{
				...
				msg_resp = memalloc(sizeof(NeonErrorResponse) + msglen + 1);
...
neon_nblocks(SMgrRelation reln, ForkNumber forknum)
{
...

	{
		...
		return (BlockNumber)communicator_request(0, &request);
	}
}
