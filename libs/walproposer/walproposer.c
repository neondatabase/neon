...
static void WalProposerInit(XLogRecPtr flushRecPtr);
...
int WalProposerRust(void) {
    elog(INFO, "This is a shared library test...");
    return 42;
}
...
{
	GetXLogReplayRecPtr();
	WalProposerInit(GetFlushRecPtr());
...
static void
WalProposerInit(XLogRecPtr flushRecPtr)
{
	char	   *host;
	char	   *sep;
	char	   *port;
	...
	for (host = wal_acceptors_list; host != NULL && *host != '\0'; host = sep)
	{
		...
		*port++ = '\0';
		...
		safekeeper[n_safekeepers].xlogreader = XLogReaderAllocate();
...
InitEventSet(void)
{
	...
	waitEvents = CreateWaitEventSet(2 + n_safekeepers);
...
WalProposerStartStreaming(XLogRecPtr startpos)
{
	...
	StartProposerReplication(startpos);
}
