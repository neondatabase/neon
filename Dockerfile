...
&& if [ -s /run/secrets/SUBZERO_ACCESS_TOKEN ]; then \
    ...
    sed -i 's/rest_broker = \["dep:subzero-core", "dep:ouroboros"\]/rest_broker = []/' proxy/Cargo.toml && \
    cargo remove -p proxy subzero-core && \
    cargo remove -p proxy ouroboros; \
fi \
...
COPY --chown=nonroot . .
COPY --from=plan     /home/nonroot/Cargo.lock                               Cargo.lock
COPY --from=plan     /home/nonroot/proxy/Cargo.toml                         proxy/Cargo.toml
