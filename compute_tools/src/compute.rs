...
fn extract_pageserver_conninfo_from_connstr(connstr: &str) -> PageserverConnectionInfo {
...
impl TryFrom<ComputeSpec> for ParsedSpec {
    ...
    fn try_from(spec: ComputeSpec) -> Result<Self, String> {
        ...
        // the storage system.
        //
        // In compute specs generated by old control plane versions, the spec fil might be
        // missing the `pageserver_connection_info` field. In that case, we need to dig
        ...
        if pageserver_conninfo.is_none() {
            if let Some(guc) = spec.cluster.settings.find("neon.pageserver_connstring") {
                pageserver_conninfo = Some(extract_pageserver_conninfo_from_connstr(&guc));
            }
        }
        let pageserver_conninfo = pageserver_conninfo
            .ok_or("pageserver connection information should be provided".to_string())?;
