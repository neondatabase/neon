...
pub(crate) struct Args {
    ...
    /// will request page batches (e.g. via prefetches or vectored reads). The RPS counts number of
    /// batches.
    ///
    /// The libpq protocol does not support client-side batching, and will submit batches as many
    /// individual requests. Each batch still counts as 1 request wrt. client queue depth and RPS.
    #[clap(long, default_value = "1")]
    ...
    while !cancel.is_cancelled() {
        ...
        while inflight.len() < args.queue_depth.get() {
            ...
            let (req_lsn, mod_lsn, rel, blks) = {
                ...
                let mut blks = VecDeque::with_capacity(args.batch_size.into());
...
struct LibpqClient {
    ...
    // libpq doesn't support client-side batches, so we send a bunch of individual requests instead
    // in the hope that the server will batch them for us. But we have to know how many responses to
    // wait for after sending a batch (the batches may hit a relation boundary), so we track that
    // here.
