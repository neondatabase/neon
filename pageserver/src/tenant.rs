...
mod tests {
    ...
    fn test_basic() -> anyhow::Result<()> {
        ...
        let tline = tenant
            .create_empty_timeline(TIMELINE_ID, Lsn(0), DEFAULT_PG_VERSION, RegionId(0))?
            .initialize()?;
    ...
    fn no_duplicate_timelines() -> anyhow::Result<()> {
        ...
        match tenant.create_empty_timeline(TIMELINE_ID, Lsn(0), DEFAULT_PG_VERSION, RegionId(0)) {
    ...
    fn test_branch() -> anyhow::Result<()> {
        ...
        tenant.branch_timeline(TIMELINE_ID, NEW_TIMELINE_ID, Some(Lsn(0x30)), RegionId(0))?;
        let newtline = tenant
    ...
    async fn test_prohibit_branch_creation_on_garbage_collected_data() -> anyhow::Result<()> {
        ...
        match tenant.branch_timeline(TIMELINE_ID, NEW_TIMELINE_ID, Some(Lsn(0x25)), RegionId(0)) {
    ...
    fn test_prohibit_branch_creation_on_pre_initdb_lsn() -> anyhow::Result<()> {
        ...
        tenant
            .create_empty_timeline(TIMELINE_ID, Lsn(0x50), DEFAULT_PG_VERSION, RegionId(0))?
            .initialize()?;
    ...
    async fn test_retain_data_in_parent_which_is_needed_for_child() -> anyhow::Result<()> {
        ...
        tenant.branch_timeline(TIMELINE_ID, NEW_TIMELINE_ID, Some(Lsn(0x40)), RegionId(0))?;
        let newtline = tenant
            .get_timeline(NEW_TIMELINE_ID, true)
            .expect("Should have a local timeline");
        // this removes layers before lsn 40 (50 minus 10), so there are two remaining layers, image and delta for 31-50
    ...
    async fn timeline_load() -> anyhow::Result<()> {
        ...
        let harness = TenantHarness::create(TEST_NAME)?;
        {
            ...
            let tline = tenant
                .create_empty_timeline(TIMELINE_ID, Lsn(0x8000), DEFAULT_PG_VERSION, RegionId(0))?
                .initialize()?;
    ...
    async fn test_traverse_branches() -> anyhow::Result<()> {
        ...
        for _ in 0..50 {
            ...
            tenant.branch_timeline(tline_id, new_tline_id, Some(lsn), RegionId(0))?;
            tline = tenant
