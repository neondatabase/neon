...
"#
        ...
        target_config_file
            ...
            .with_context(|| {
                format!(
                    "Failed to write config file bytes into path '{}'",
    ...
    fn branch_timeline(
        ...
        let new_timeline =
            self.create_initial_timeline_files(dst, metadata, target_timeline_dir)?;
    ...
    async fn bootstrap_timeline(
        ...
        let initdb_lsn = import_datadir::get_lsn_from_controlfile(pgdata_path)?.align();

        // Import the contents of the data directory at the initial checkpoint LSN, and any WAL after that.
    ...
    fn create_initial_timeline_files(
        &self,
        new_timeline_id: TimelineId,
        new_metadata: TimelineMetadata,
        target_dir: &Path,
        ...
        save_metadata(
            &self.conf.construct_timeline_metadata_path(target_dir),
            &new_metadata,
            true,
        )?;
...
async fn run_initdb(conf: &'static PageServerConf, initdbpath: &Path) -> Result<()> {
...
mod tests {
    ...
    async fn test_prohibit_branch_creation_on_pre_initdb_lsn() -> Result<()> {
        ...
        match tenant.branch_timeline(
            ...
            NEW_TIMELINE_ID,
            Some(Lsn(0x25)),
            &tenant.conf.timeline_path(&TIMELINE_ID, &tenant.tenant_id),
        ) {
            Ok(_) => panic!("branching should have failed"),
            Err(err) => {
                assert!(&err.to_string().contains("invalid branch start lsn"));
