...
pub enum Value {
    ...
    /// Lineages -- many differential records serialized into one entry.
    /// Needs specialized handling in read/write, and doesn't occur naturally:
    /// this is generated by rolling up many `WalRecord` and up to one `Image`
    /// variants of this enum.
    NonInitiatingLineage(Lineage),
    InitiatingLineage(Lineage),
}
...
impl Value {
    ...
    pub fn will_init(&self) -> bool {
        match self {
            ...
            Value::NonInitiatingLineage(_) => false,
            Value::InitiatingLineage(_) => true,
        }
    }
    ...
    pub fn guestimate_size(&self) -> usize {
        size_of::<u64>()
            + match self {
                Value::Image(it) => it.len(),
                Value::WalRecord(rec) => rec.guestimate_size(),
                Value::NonInitiatingLineage(lin) => lin.guestimate_size(),
                Value::InitiatingLineage(lin) => lin.guestimate_size(),
            }
    }
}
