...
async fn shutdown_all_tenants0(tenants: &std::sync::RwLock<TenantsMap>) {
    ...
    info!(
        "Waiting for {} InProgress tenants to complete...",
        in_progress_ops.len()
    );
    for notify in in_progress_ops {
        notify.notified().await;
    }

    info!(
        "Waiting for {} attached tenants to shut down...",
...
pub(crate) async fn create_tenant(
    ...
    if tenant_id != created_tenant_id {
        return Err(TenantMapInsertError::Other(anyhow::anyhow!("loaded created tenant has unexpected tenant id (expect {tenant_id} != actual {created_tenant_id})")));
    }
...
pub(crate) async fn load_tenant(
    ...
    .with_context(|| format!("schedule tenant processing in path {tenant_path:?}"))?;
...
pub(crate) async fn attach_tenant(
    ...
    let location_conf = LocationConf::attached_single(tenant_conf, generation);
    let tenant_dir = create_tenant_files(
        conf,
        &location_conf,
        &tenant_id,
...
fn tenant_map_acquire_slot_impl(
    ...
    match entry {
        Entry::Vacant(v) => {
            ...
            v.insert(TenantSlot::InProgress(notify.clone()));
            tracing::debug!("Vacant, inserted InProgress");
            Ok(SlotGuard::new(*tenant_id, None, notify))
        }
        Entry::Occupied(mut o) => {
            ...
            let notify = Arc::new(tokio::sync::Notify::new());
            let old_value = o.insert(TenantSlot::InProgress(notify.clone()));
            tracing::debug!("Occupied, replaced with InProgress");
            Ok(SlotGuard::new(*tenant_id, Some(old_value), notify))
        }
    }
}
