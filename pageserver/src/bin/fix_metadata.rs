    ...
    let arg_matches = App::new("Neon update metadata utility")
        ...
        .arg(
            Arg::new("write")
                .short('w')
                .long("write")
                .help("write changed metadata"),
        )
        .get_matches();

    let path = PathBuf::from(arg_matches.value_of("path").unwrap());
    let metadata_bytes = std::fs::read(&path)?;
    let meta = TimelineMetadata::from_bytes(&metadata_bytes)?;
    println!("Current metadata:\n{:?}", &meta);

    let update_meta = arg_matches.is_present("write");

    if update_meta {
        let metadata_bytes = meta.to_bytes_v1()?;
