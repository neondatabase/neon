...
// Another
// example: the initial tenant loading tasks are launched in the background ops runtime.
// If a GetPage request comes in before the load of a tenant has finished, the GetPage
// request will wait for the tenant load to finish.
//
...
pub async fn shutdown_watcher() {
    let mut shutdown_rx = SHUTDOWN_RX
        ...
        .expect("shutdown_requested() called in an unexpected task");
...
pub fn is_shutdown_requested() -> bool {
    if let Ok(shutdown_rx) = SHUTDOWN_RX.try_with(|rx| rx.clone()) {
        *shutdown_rx.borrow()
        ...
        if !cfg!(test) {
            warn!("is_shutdown_requested() called in an unexpected task");
        }
        false
    }
}
