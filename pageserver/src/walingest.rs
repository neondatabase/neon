...
impl WalIngest {
    ...
    ///
    // TODO(vlad): this function will take a buffer for the ephemeral file and series of metadata
    // modifications to perform. We first write to the file (crash if we fail) and apply
    // modifications afterwards.
    //
    // Metadata modifications can also fail. What to do about them?
    // We advance `last_record_lsn` after ingesting the whole thing (buffer + meta).
    // If we fail, don't advance `last_record_lsn`. This causes us to resubscribe and try again.
    // We should somehow get rid of the current in-memory layer such that we don't dump more shit
    // into it.
    ...
    async fn ingest_multixact_zero_page_record(
        ...
        let MultiXactZeroPage { segno, rpageno } = zero_page;
        self.put_slru_page_image(
            modification,
            SlruKind::MultiXactOffsets,
            segno,
