...
impl<'a> WalIngest<'a> {
    ...
    pub fn ingest_record(
        ...
        } else if decode.xl_rmid == pg_constants::RM_XLOG_ID {
            let info = decoded.xl_info & !pg_constants::XLR_INFO_MASK;
    ...
    fn ingest_xact_record(
        ...
        let mut rpageno = csn_pageno % pg_constants::SLRU_PAGES_PER_SEGMENT;
        let mut csn_page_xids: Vec<TransactionId> = vec![parsed.xid];
        let mut lsn : XidCSN = modification.lsn;
        ...
        for subxact in &parsed.subxacts {
            ...
            if csn_subxact_pageno != csn_pageno {
                ...
                modification.put_slru_wal_record(
                    ...
                    csn_rpageno,
                    if is_commit {
                        ZenithWalRecord::CsnLogSetCommitted { xids: csn_page_xids, lsn: lsn }
                    } else {
                        ZenithWalRecord::CsnLogSetAborted { xids: csn_page_xids }
                    },
                )?;
