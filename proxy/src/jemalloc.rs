    ...
    Metrics: MetricGroup<Enc>,
{
    fn collect_into(&self, enc: &mut Enc) {
        if self.epoch.advance().is_ok() {
            MetricGroup::collect_into(&self.inner, enc);
        }
    }
}
...
impl<M, T> MetricFamilyEncoding<T> for JemallocGaugeFamily<M>
where
    JemallocGauge<M>: MetricEncoding<T, Metadata = M>,
{
    fn collect_into(&self, name: impl MetricNameEncoder, enc: &mut T) {
        ...
        <JemallocGauge<M> as MetricEncoding<T>>::collect_into(
            &JemallocGauge(PhantomData),
            &self.0,
            NoLabels,
            name,
...
macro_rules! jemalloc_gauge {
    ($stat:ident, $mib:ident) => {
        impl MetricEncoding<TextEncoder> for JemallocGauge<stats::$mib> {
            ...
            fn collect_into(
                ...
                if let Ok(v) = mib.read() {
                    enc.write_metric(name, labels, MetricValue::Int(v as i64));
                }
            }
        }
    };
