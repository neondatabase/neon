...
impl<S> PqStream<S> {
    ...
    pub fn new_skip_handshake(stream: S) -> Self {
        Self {
            ...
            read: Vec::new(),
            write: WriteBuf::new(),
            tracker: tokio_util::task::TaskTracker::new().token(),
        }
    }
}
...
impl<S: AsyncRead + AsyncWrite + Unpin> PqStream<S> {
    ...
    pub async fn parse_startup(
        mut stream: S,
        tracker: TaskTrackerToken,
    ) -> io::Result<(Self, FeStartupPacket)> {
...
impl<S: AsyncWrite + Unpin> PqStream<S> {
    ...
    /// This is not cancel safe
    pub async fn accept_tls(mut self) -> io::Result<(S, TaskTrackerToken)> {
        // S for SSL.
    ...
    pub fn accept_direct_tls(self) -> (S, TaskTrackerToken) {
        (self.stream, self.tracker)
    }

    /// Assert that the client connection is for canellation.
    pub fn cancelled(self) -> TaskTrackerToken {
        self.tracker
    }
