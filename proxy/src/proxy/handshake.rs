...
pub(crate) async fn handshake<S: AsyncRead + AsyncWrite + Unpin>(
    ...
    let (mut stream, mut msg) = PqStream::parse_startup(Stream::from_raw(stream), tracker).await?;
    loop {
        match msg {
            FeStartupPacket::SslRequest { direct } => match stream.get_ref() {
                Stream::Raw { .. } if !tried_ssl => {
                    ...
                    if let Some(tls) = tls.take() {
                        ...
                        let mut read_buf;
                        let (raw, tracker) = if let Some(direct) = &direct {
                            read_buf = &direct[..];
                        ...
                        (stream, msg) = PqStream::parse_startup(tls, tracker).await?;
                    } else {
                        if direct.is_some() {
            ...
            FeStartupPacket::CancelRequest(cancel_key_data) => {
                ...
                break Ok(HandshakeData::Cancel(cancel_key_data, stream.cancelled()));
            }
        }
    }
}
